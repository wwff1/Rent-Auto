...
router.post("/add", async function (req, res) {
    ...
    const date = req.body.dateA
    const auto = {_id: mongoose.Types.ObjectId(req.body.model.id), name: req.body.model.name}
    ...
    // const candidate = await Accident.findOne({date, model, body, clas, park, sum })
    // if (candidate){
    //     return res.status(400).json({message: "Такая марка уже существует"})
    // }
    // const accident = new Accident({date, model, body, clas, park, sum })
    // await accident.save()
});
